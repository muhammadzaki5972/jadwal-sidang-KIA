...
const PemohonPage = () => {
  ...
  const [formData, setFormData] = useState({ id: null, nama_pemohon: '', kategori_pemohon: 'Individu' });

  const fetchData = async () => {
    try {
      const res = await api.get('/pemohon');
      setData(res.data);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formData.id) {
        await api.put(`/pemohon/${formData.id}`, formData);
        toast.success('Data berhasil diubah');
      } else {
        await api.post('/pemohon', formData);
  ...
  const openModal = (item = null) => {
    if (item) {
      ...
      setFormData({ id: null, nama_pemohon: '', kategori_pemohon: 'Individu' });
    }
    setIsModalOpen(true);
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-base-content">Data Pemohon</h1>
          <p className="text-base-content/60 mt-1">Kelola data pihak pemohon sengketa</p>
        </div>
        <button onClick={() => openModal()} className="btn btn-primary shadow-sm">
          <Plus size={20} /> Tambah Pemohon
        </button>
      </div>
              ...
              {loading ? (
                ...
                data.map((item, index) => (
                  <tr key={item.id}>
                    ...
                    <td className="font-semibold">{item.nama_pemohon}</td>
                    <td>
                      <div className="badge badge-info gap-2 badge-sm py-3 px-3 font-semibold">
      ...
      <dialog className={`modal ${isModalOpen ? 'modal-open' : ''}`}>
        <div className="modal-box">
          <h3 className="font-bold text-lg mb-6 border-b pb-4">
            {formData.id ? 'Edit Pemohon' : 'Tambah Pemohon'}
          </h3>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="form-control w-full">
              <label className="label">
                <span className="label-text font-semibold">Nama Pemohon</span>
              </label>
              <input
                type="text"
                required
